...
<body>
			...
			<ul class="pagination">
				<c:if test="${pageMaker.prev}">
					<li class="page-item"><a class="page-link"
						href="<%=pageContext.getServletContext().getContextPath()%>/proposal/waitingApprovalList?page=${pageMaker.startPage -1}&perPageNum=${cri.getPerPageNum()}&p_status=결재대기&p_acceptor=<%=obj.getName() %>"> &laquo;</a>
					</li>
				</c:if>
				<c:forEach begin="${pageMaker.startPage }"
					...
					<li class="page-item"
						...
						<a class="page-link" href="<%=pageContext.getServletContext().getContextPath()%>/proposal/waitingApprovalList?page=${idx}&perPageNum=${cri.getPerPageNum()}&p_status=결재대기&p_acceptor=<%=obj.getName() %>">${idx}</a>
					</li>
				</c:forEach>
				<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
					<li class="page-item"><a class="page-link"
						href="<%=pageContext.getServletContext().getContextPath()%>/proposal/waitingApprovalList?page=${pageMaker.endPage+1}&perPageNum=${cri.getPerPageNum()}&p_status=결재대기&p_acceptor=<%=obj.getName() %>">&raquo;</a>
					</li>
				</c:if>
			</ul>
		</div>
	...
	<script type="text/javascript">
		...
		$("#searchBtn").on("click", function(event){
			self.location = "<%=pageContext.getServletContext().getContextPath()%>/proposal/waitingApprovalList?page=1&perPageNum=10&searchType=" + $("select option:selected").val() + "&keyword=" + $('#keywordInput').val()+"&p_status=결재대기&p_acceptor=" + "<%=obj.getName() %>";
		});
	</script>
</body>
</html>